...
// export const getListMessagesByRecipe = async (req: Request, res: Response): Promise<Response> => {

//     try {

//         const conn = await connect();

//         const listdb = await conn.query<RowDataPacket[]>(`CALL SP_GET_ALL_MESSAGE_BY_RECIPE(?);`,[req.idPerson]);

//         conn.end();

//         return res.json({
//             resp: true,
//             message: 'Messenger of recipe',
//             listChat: listdb[0][0]
//         });
...
// INSERT INTO `list_recipe_chat`(`uid`, `soure_uid`, `recipe_uid`, `last_message`, `update_at`) VALUES ()
        await conn.query(
          'INSERT INTO list_recipe_chat (uid, source_uid, recipe_uid) VALUE (?,?,?)',
...
export const updateLastMessageRecipe = async (uidTarget: string, uidPerson: string, message: string) => {
